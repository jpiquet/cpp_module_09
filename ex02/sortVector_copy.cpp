...
/*   Updated: 2026/09/18 14:16:34 by jocelyn          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::vector<std::vector<int> > sortVec( std::vector<std::vector<int> > list)
{
	...
	if (hasStraggler)
	{
		...
		// std::cout << straggler[0] << std::endl;

		insertPos = std::lower_bound(mainChain.begin(), mainChain.end(), straggler);
		mainChain.insert(insertPos, straggler);
	}
	// printPairs(mainChain);

	return mainChain;
}

// std::vector<std::vector<int> > binarySearch(std::vector<int> winner, std::vector<int> loser)
// {
